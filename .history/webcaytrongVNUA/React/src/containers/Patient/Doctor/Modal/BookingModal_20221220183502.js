...
class BookingModal extends Component {
    ...
    async componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (this.props.dataTime !== prevProps.dataTime) {
            if (this.props.dataTime && !_.isEmpty(this.props.dataTime)) {
                ...
                let priceId = this.props.dataTime.price
                this.setState({
                    doctorId: doctorId,
                    timeType: timeType,
 ...
 // bất kỳ thứ gì chèn vào đầu vào đều có thể được truy cập thông qua event.target.value
    ...
    buildInfoBooking = (dataTime) => {
        ...
        if (dataTime && !_.isEmpty(dataTime)) {
            ...
            let price = language === LANGUAGES.VI ?
                `${dataTime.priceTypeData.valueVi} `
                :
                ` ${dataTime.priceTypeData.valueEn}`;
            return `${time} - ${date} -${name} - ${price}`;
    ...
    handleConfirmBooking = async () => {
        ...
        let infoString = this.buildInfoBooking(this.props.dataTime);
