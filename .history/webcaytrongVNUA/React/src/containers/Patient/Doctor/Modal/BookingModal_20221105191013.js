...
class BookingModal extends Component {
 ...
 // bất kỳ thứ gì chèn vào đầu vào đều có thể được truy cập thông qua event.target.value
    ...
    handleOnChangeDatePickers =(dates) =>{
        this.setState({
    ...
    buildTimeBooking = (dataTime) => {
        ...
        if (dataTime && !_.isEmpty(dataTime)) {
            ...
            let dates = language === LANGUAGES.VI ?
                moment.unix(+dataTime.dates / 1000).format('dddd -DD/MM/YYYY') :
                moment.unix(+dataTime.dates / 1000).locale('en').format('ddd- MM/DD/YYYY');
            return `${time} - ${date}`;
        }
        return ''
    }
    ...
    handleConfirmBooking = async () => {
        ...
        let dates = new Date(this.state.birthdays).getTime();
