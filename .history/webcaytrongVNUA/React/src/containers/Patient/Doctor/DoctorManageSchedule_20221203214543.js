...
class DoctorManageSchedule extends Component {
  ...
  async  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    handleChangeSelect = async (selectedOption) => {
        this.setState({ userInfo: selectedOption });
    }
    ...
    handleSaveSchedule = async () => {
        let { rangeTime, userInfo, currentDate } = this.state;
        let result = [];
        if (!currentDate) {
        ...
        if (rangeTime && rangeTime.length > 0) {
            ...
            if (selectedTime && selectedTime.length > 0) {
                selectedTime.map((schedule, index) => {
                    ...
                    let object = {};
                    object.doctorId = userInfo.value;
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-6 form-group">
                            ...
                            <Select
                                value={userInfo && userInfo.firstName && userInfo.lastName ? ' ' + userInfo.firstName + ' ' + userInfo.lastName : ' '}
                                onChange={this.handleChangeSelect}
                                options={userInfo && userInfo.firstName && userInfo.lastName ? ' ' + userInfo.firstName + ' ' + userInfo.lastName : ' '}
                            />
