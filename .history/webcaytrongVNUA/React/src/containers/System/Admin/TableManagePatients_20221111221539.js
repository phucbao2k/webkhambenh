...
class TableManagePatients extends Component {
    ...
    renderTimeBooking = (ar) => {
        let { language } = this.props;
        if (ar && !_.isEmpty(ar)) {
            // let time = language === LANGUAGES.VI ?
            //     ar.timeTypeData.valueVi : ar.timeTypeData.valueEn;
            let date = language === LANGUAGES.VI ?
                
           moment.unix(+ar.date / 1000).format('dddd -DD/MM/YYYY') 
                :
                moment.unix(+ar.date / 1000).locale('en').format('ddd -MM/DD/YYYY')
