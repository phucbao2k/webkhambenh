...
class TableManagePatients extends Component {
    ...
    renderTimeBooking = (listBookings) => {
        let { language } = this.props;
        if (listBookings && !_.isEmpty(listBookings)) {
            // let time = language === LANGUAGES.VI ?
            //     listBookings.timeTypeData.valueVi : listBookings.timeTypeData.valueEn;
            let date = language === LANGUAGES.VI ?
                
           moment.unix(+listBookings.date / 1000).format('dddd -DD/MM/YYYY') 
                :
                moment.unix(+listBookings.date / 1000).locale('en').format('ddd -MM/DD/YYYY')
                            ...
                            <tbody>
                                ...
                                {arrBookings && arrBookings.length > 0 && arrBookings.map((item, index) => {
                                    ...
                                    return (
                                        <tr key={index}>
                                            <td>{index + 1}</td>
                                            <td>{this.renderTimeBooking(listBookings)}</td>
