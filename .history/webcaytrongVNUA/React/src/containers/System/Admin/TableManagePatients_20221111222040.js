...
class TableManagePatients extends Component {
    ...
    renderTimeBooking = (lis) => {
        let { language } = this.props;
        if (lis && !_.isEmpty(lis)) {
            // let time = language === LANGUAGES.VI ?
            //     lis.timeTypeData.valueVi : lis.timeTypeData.valueEn;
            let date = language === LANGUAGES.VI ?
                
           moment.unix(+lis.date / 1000).format('dddd -DD/MM/YYYY') 
                :
                moment.unix(+lis.date / 1000).locale('en').format('ddd -MM/DD/YYYY')
