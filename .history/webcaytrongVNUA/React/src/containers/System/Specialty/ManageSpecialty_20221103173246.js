...
class ManageSpecialty extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.specialityRedux !== this.props.specialityRedux) {
            let arrSpecial = this.props.specialityRedux;
            this.setState({
                specialityArr: arrSpecial,
                speciality: arrSpecial && arrSpecial.length > 0 ? arrSpecial[0].keyMap : '',
            })
        }
        if (prevProps.listSpecial !== this.props.listSpecial) {
            let arrSpecial = this.props.specialityRedux;
            this.setState({
                name: '',
                address: '',
                specialty: arrSpecial && arrSpecial.length > 0 ? arrSpecial[0].keyMap : '',
                avatar: '',
    ...
    handleSaveSpecialty = () => {
        ...
        if (action === CRUD_ACTIONS.EDIT) {
            this.props.editSpecialtyRedux({
                id: this.state.specialityEditId,
                name: this.state.name,
                address: this.state.address,
                avatar: this.state.avatar,
                descriptionHTML: this.state.descriptionHTML,
    ...
    handleEditSpecialtyFromParent = (speciality) => {
        let imageBase64 = '';
        if (speciality.image) {
            imageBase64 = new Buffer(speciality.image, 'base64').toString('binary');
        ...
        this.setState({
            ...
            specialityEditId: speciality.id,
            descriptionHTML: speciality.descriptionHTML,
            descriptionMarkdown: speciality.descriptionMarkdown
        })
    }
    ...
    render() {
        ...
        return (
            ...
            <div className="user-redux-container" >
                <div className="title">
                    <FormattedMessage id="menu.admin.manage-speciality" />
                </div>
...
const mapStateToProps = state => {
    return {
        ...
        listSpecial: state.admin.specialitys
    };
};
