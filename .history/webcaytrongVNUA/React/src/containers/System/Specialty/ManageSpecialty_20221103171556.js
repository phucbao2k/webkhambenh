...
class ManageSpecialty extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.clinicRedux !== this.props.clinicRedux) {
            let arrSpecialtys = this.props.clinicRedux;
            this.setState({
                clinicArr: arrSpecialtys,
                clinic: arrSpecialtys && arrSpecialtys.length > 0 ? arrSpecialtys[0].keyMap : '',
            })
        }
    ...
    handleEditSpecialtyFromParent = (clinic) => {
        let imageBase64 = '';
        if (clinic.image) {
...
const mapStateToProps = state => {
    return {
        ...
        listSpecialtys: state.admin.clinics
    };
};
