...
import {
    ...
    editClinicService, createNewClinicService, createNewSpecialtyService, deleteSpecialtyService, editSpecialtyService
...
export const deleteSpecialty = (specId) => {
    return async (dispatch, getState) => {
        try {
            let res = await deleteSpecialtyService(specId);
            if (res && res.errCode === 0) {
                toast.success('Successfully deleted spec ');
                dispatch(deleteSpecialtySuccess());
                dispatch(fetchAllSpecialtyStart());

            } else {
                toast.error("Delete the spec error!");
                dispatch(deleteSpecialtyFailed());
            }
        } catch (e) {
            toast.error("Delete the spec error!");
            dispatch(deleteSpecialtyFailed());
            console.log('delete spec Failed', e)
        }
    }
}
