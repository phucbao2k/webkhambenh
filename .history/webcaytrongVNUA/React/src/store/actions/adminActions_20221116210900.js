...
import {
    ...
    createNewHandbookService, deleteHandbookService, editHandbookService, showAllHandbooks, deleteBooking, getAllBookings, getAllBookingForPatient
...
//USER-BOOKING
export const fetchAllBookingPatientStart = (data) => {
    return async (dispatch, getState) => {
        try {
            let res = await getAllBookingForPatient(data);
            console.log('check data bookings: ', res);
            if (res && res.errCode === 0) {
                dispatch(fetchAllPatientBookingsSuccess(res.patientBookings.reverse()))
            } else {
                toast.error("Failed to fetch all bookings");
                dispatch(fetchAllPatientBookingsFailed());
            }
        } catch (e) {
            toast.error("Failed to fetch all bookings");
            dispatch(fetchAllPatientBookingsFailed());
...
export const deleteBookingPatientService = (bookingId) => {
    return async (dispatch, getState) => {
        try {
            let res = await deletePatientBooking(bookingId);
            if (res && res.errCode === 0) {
                toast.success('Successfully deleted booking ');
