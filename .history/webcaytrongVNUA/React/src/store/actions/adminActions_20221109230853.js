...
import {
    ...
    createNewHandbookService, deleteHandbookService, editHandbookService, showAllHandbooks, getAllBookingForAdmin, deleteBooking
...
export const fetchAllBookingsStart = () => {
    return async (dispatch, getState) => {
        try {
            let res = await getAllBookingForAdmin(data);
            let res1 = await getTopDoctorHomeService('');
            console.log("check top: ", res1);
            if (res && res.errCode === 0) {
                dispatch(fetchAllBookingsSuccess(res.bs.reverse()))
            } else {
                toast.error("Failed to fetch all bs");
                dispatch(fetchAllBookingsFailed());
            }
        } catch (e) {
            toast.error("Failed to fetch all bs");
            dispatch(fetchAllBookingsFailed());
            console.log("check fetch bs failed: ", e);
        }
    }
};
...
export const deleteBooking = (bId) => {
    return async (dispatch, getState) => {
        try {
            let res = await deleteBooking(bId);
            if (res && res.errCode === 0) {
                toast.success('Successfully deleted b ');
                dispatch(deleteBookingSuccess());
                dispatch(fetchAllBookingsStart());

            } else {
                toast.error("Delete the b error!");
