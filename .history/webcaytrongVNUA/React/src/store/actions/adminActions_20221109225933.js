...
import {
    ...
    createNewHandbookService, deleteHandbookService, editHandbookService, showAllHandbooks
...
//BOOKING
export const fetchAllBsStart = () => {
    return async (dispatch, getState) => {
        try {
            let res = await getAllBookingForAdmin("ALL");
            let res1 = await getTopDoctorHomeService('');
            console.log("check top: ", res1);
            if (res && res.errCode === 0) {
                dispatch(fetchAllBsSuccess(res.users.reverse()))
            } else {
                toast.error("Failed to fetch all users");
                dispatch(fetchAllBsFailed());
            }
        } catch (e) {
            toast.error("Failed to fetch all users");
            dispatch(fetchAllBsFailed());
...
export const fetchAllBsSuccess = (data) => ({
    type: actionTypes.FETCH_ALL_USER_SUCCESS,
    users: data
})
export const fetchAllBsFailed = () => ({
    type: actionTypes.FETCH_ALL_USER_FAILED,
})
export const deleteB = (userId) => {
    return async (dispatch, getState) => {
        try {
            let res = await deleteBService(userId);
            if (res && res.errCode === 0) {
                toast.success('Successfully deleted user ');
                dispatch(deleteBSuccess());
                dispatch(fetchAllBsStart());

            } else {
                toast.error("Delete the user error!");
                dispatch(deleteBFailed());
            }
        } catch (e) {
            toast.error("Delete the user error!");
            dispatch(deleteBFailed());
            console.log('delete B Failed', e)
        }
    }
}
export const deleteBSuccess = () => ({
    type: actionTypes.DELETE_USER_SUCCESS
})
export const deleteBFailed = () => ({
    type: actionTypes.DELETE_USER_FAILED
});
//CLINIC
