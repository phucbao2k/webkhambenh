...
import {
    ...
    editClinicService, createNewClinicService
...
export const createNewSpeci = (data) => {
    return async (dispatch, getState) => {
        try {
            let res = await createNewSpeciService(data);
            console.log(' check create user redux: ', res)
            if (res && res.errCode === 0) {
                toast.success("Create a new user succeed!")
                dispatch(saveSpeciSuccess());
                dispatch(fetchAllSpeciStart());
            } else {
                dispatch(saveSpeciFailed());
            }
        } catch (e) {
            dispatch(saveSpeciFailed());
            console.log('check err: ', e)
...
export const saveSpeciSuccess = () => ({
    type: 'CREATE_CLINIC_SUCCESS',
})
export const saveSpeciFailed = () => ({
    type: 'CREATE_CLINIC_FAILED',
});
