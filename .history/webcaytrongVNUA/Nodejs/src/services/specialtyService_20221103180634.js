...
let deletesPE = (specialtyId) => {
    return new Promise(async (resolve, reject) => {
        let foundsPE = await db.Speciality.findOne({
            where: { id: specialtyId }
        })
        if (!foundsPE) {
            resolve({
...
let updatesPEData = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let clinic = await db.Speciality.findOne({
                where: { id: data.id },
                raw: false
            })
            if (clinic) {
