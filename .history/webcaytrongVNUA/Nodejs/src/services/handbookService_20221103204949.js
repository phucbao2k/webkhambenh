...
let deleteHan = (handbookId) => {
    return new Promise(async (resolve, reject) => {
        let foundHan = await db.Speciality.findOne({
            where: { id: handbookId }
        })
        if (!foundHan) {
            resolve({
...
module.exports = {
    createHan: createHan,
    getAllHan: getAllHan,
    getDetailHanById: getDetailHanById,
    showAllSpecialties: showAllSpecialties,
    deleteHan: deleteHan,
    updateHanData: updateHanData
