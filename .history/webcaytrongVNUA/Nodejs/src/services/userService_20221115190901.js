    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (check === true && phoneNumber ===true) {
                ...
                let user = await db.User.findOne({
                    attributes: ['id', 'email', 'roleId', 'password', 'firstName', 'lastName', 'phone'],
                    where: { email: email },
                    raw: true,
...
let handleUserRegister = (email, password) => {
    return new Promise(async (resolve, reject) => {
        try {
            //kiểm tra user đã tồn tại hay chưa
            let check = await checkUserEmail(data.email)
            let pass = await checkUserPassword(data.password)
            if (check === true && pass === false) {
                let hashPasswordFromBcrypt = await hashUserPassword(data.password);
                let user = await db.User.findOne({
                    attributes: ['id', 'email', 'roleId', 'password', 'firstName', 'lastName'],
                    where: { email: email },
                ...
                resolve({
                    errCode: 1,
                    errMessage: `User's is registered!`
                });
            }
        } catch (e) {
            reject(e);
