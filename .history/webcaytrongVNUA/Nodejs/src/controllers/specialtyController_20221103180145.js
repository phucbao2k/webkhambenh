...
let handleDeleteSpecial = async (req, res) => {
    if (!req.body.id) {
        return res.status(200).json({
            errCode: 1,
            errMessage: "Missing required parameters!"
    ...
    let message = await specialtyService.deleteSpecial(req.body.id);
    return res.status(200).json(message);
}
let handleEditSpecial = async (req, res) => {
    let data = req.body;
    let message = await specialtyService.updateSpecialData(data);
    return res.status(200).json(message);
}
module.exports = {
