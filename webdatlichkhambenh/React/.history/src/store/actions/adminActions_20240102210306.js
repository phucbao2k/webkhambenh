...
import {
    ...
    saveScheduleDoctorService, getUserNotDoctors, saveDetailUserNotDoctorService, getDetailInforUserNotDoctor, getUsersByRoleId,
    getAdminByRoleId, getAdminBookingByRoleId, getDoctorByRoleId
...
export const updateDoctorDetails = (details) => ({
    type: 'UPDATE_DOCTOR_DETAILS',
    payload: details,
});
export const fetchUserNotDoctor = () => {
    return async (dispatch, getState) => {
        try {
            let res = await getUserNotDoctors();

            if (res && res.errCode === 0) {
                dispatch({
                    type: actionTypes.FETCH_USER_NOT_DOCTOR_SUCCESS,
                    dataUsers: res.data
                ...
                dispatch({
                    type: actionTypes.FETCH_USER_NOT_DOCTOR_FAILED
                })
            }
        } catch (e) {
            console.log('FETCH_USER_NOT_DOCTORS_FAILED', e)
            dispatch({
                type: actionTypes.FETCH_USER_NOT_DOCTORS_FAILED
            })
        }
    }
}
export const saveDetailUserNotDoctor = (data) => {
    return async (dispatch, getState) => {
        try {
            let res = await saveDetailUserNotDoctorService(data);

            if (res && res.errCode === 0) {
                toast.success("Update success!");
                dispatch({
                    type: actionTypes.SAVE_DETAIL_USER_NOT_DOCTOR_SUCCESS,
                    dataDocs: res.data
                })
                ...
                toast.error('Error');
                dispatch({
                    type: actionTypes.SAVE_DETAIL_USER_NOT_DOCTOR_FAILED
                })
            }
        } catch (e) {
            toast.error('Error');
            console.log('SAVE_DETAIL_USER_NOT_DOCTOR_FAILED', e)
            dispatch({
                type: actionTypes.SAVE_DETAIL_USER_NOT_DOCTOR_FAILED
            })
        }
...
export const getRequiredUserNotDoctorInfor = () => {
    return async (dispatch, getState) => {
        try {
            dispatch({ type: actionTypes.FETCH_REQUIRED_USER_NOT_DOCTOR_INFOR_START })
            ...
            if (resPrice && resPrice.errCode === 0
                ...
                dispatch(fetchRequiredUserNotDoctorInforSuccess(data))
            } else {
                dispatch(fetchRequiredUserNotDoctorInforFailed());
            }
...
export const fetchRequiredUserNotDoctorInforSuccess = (allRequiredUserData) => ({
    type: actionTypes.FETCH_REQUIRED_USER_NOT_DOCTOR_INFOR_SUCCESS,
    data: allRequiredUserData
})
export const fetchRequiredUserNotDoctorInforFailed = () => ({
    type: actionTypes.FETCH_REQUIRED_USER_NOT_DOCTOR_INFOR_FAILED,
})
...
// actions.js
export const fetchUsersByRoleId = (roleId) => {
    return async (dispatch) => {
        try {
            const res = await getUsersByRoleId(roleId);
            if (res && res.errCode === 0) {
                dispatch(fetchUsersByRoleIdSuccess(res.users));
            } else {
                // Kiểm tra nếu có thông báo lỗi từ API, sử dụng res.errMessage (nếu có)
                const errorMessage = res && res.errMessage ? res.errMessage : "Failed to fetch users by roleId R3";
                toast.error(errorMessage);
                dispatch(fetchUsersByRoleIdFailed());
            }
        } catch (error) {
            toast.error("Failed to fetch users by roleId R3");
            dispatch(fetchUsersByRoleIdFailed());
            console.error("Error fetching users by roleId R3: ", error);
        }
    };
};


export const fetchUsersByRoleIdSuccess = (data) => ({
    type: actionTypes.FETCH_USERS_BY_ROLE_ID_SUCCESS,
    users: data,
});

export const fetchUsersByRoleIdFailed = () => ({
    type: actionTypes.FETCH_USERS_BY_ROLE_ID_FAILED,
});
export const fetchAdminByRoleId = (roleId) => {
    return async (dispatch) => {
        try {
            const res = await getAdminByRoleId(roleId);
            if (res && res.errCode === 0) {
                dispatch(fetchAdminByRoleIdSuccess(res.admin));
            } else {
                // Kiểm tra nếu có thông báo lỗi từ API, sử dụng res.errMessage (nếu có)
                const errorMessage = res && res.errMessage ? res.errMessage : "Failed to fetch users by roleId R1";
                toast.error(errorMessage);
                dispatch(fetchAdminByRoleIdFailed());
            }
        } catch (error) {
            toast.error("Failed to fetch Admin by roleId R1");
            dispatch(fetchAdminByRoleIdFailed());
            console.error("Error fetching admin by roleId R1: ", error);
        }
    };
};


export const fetchAdminByRoleIdSuccess = (data) => ({
    type: actionTypes.FETCH_ADMIN_BY_ROLE_ID_SUCCESS,
    admin: data,
});

export const fetchAdminByRoleIdFailed = () => ({
    type: actionTypes.FETCH_ADMIN_BY_ROLE_ID_FAILED,
});
...
export const fetchAdminBookingByRoleId = (roleId) => {
    return async (dispatch) => {
        try {
            const res = await getAdminBookingByRoleId(roleId);
            if (res && res.errCode === 0) {
                dispatch(fetchAdminBookingByRoleIdSuccess(res.adminBooking));
            } else {
                // Kiểm tra nếu có thông báo lỗi từ API, sử dụng res.errMessage (nếu có)
                const errorMessage = res && res.errMessage ? res.errMessage : "Failed to fetch users by roleId R4";
                toast.error(errorMessage);
                dispatch(fetchAdminBookingByRoleIdFailed());
            }
        } catch (error) {
            toast.error("Failed to fetch Admin by roleId R4");
            dispatch(fetchAdminBookingByRoleIdFailed());
            console.error("Error fetching admin by roleId R4: ", error);
        }
    };
};


export const fetchAdminBookingByRoleIdSuccess = (data) => ({
    type: actionTypes.FETCH_ADMIN_BOOKING_BY_ROLE_ID_SUCCESS,
    adminBooking: data,
});

export const fetchAdminBookingByRoleIdFailed = () => ({
    type: actionTypes.FETCH_ADMIN_BOOKING_BY_ROLE_ID_FAILED,
});
//CLINIC
...
//DOCTOR
export const fetchDoctorByRoleId = (roleId) => {
    return async (dispatch) => {
        try {
            const res = await getDoctorByRoleId(roleId);
            if (res && res.errCode === 0) {
                dispatch(fetchDoctorByRoleIdSuccess(res.doctors));
            } else {
                // Kiểm tra nếu có thông báo lỗi từ API, sử dụng res.errMessage (nếu có)
                const errorMessage = res && res.errMessage ? res.errMessage : "Failed to fetch users by roleId R2";
                toast.error(errorMessage);
                dispatch(fetchDoctorByRoleIdFailed());
            }
        } catch (error) {
            toast.error("Failed to fetch Doctor by roleId R2");
            dispatch(fetchDoctorByRoleIdFailed());
            console.error("Error fetching Doctor by roleId R2: ", error);
        }
    };
};


export const fetchDoctorByRoleIdSuccess = (data) => ({
    type: actionTypes.FETCH_DOCTOR_BY_ROLE_ID_SUCCESS,
    doctors: data,
});

export const fetchDoctorByRoleIdFailed = () => ({
    type: actionTypes.FETCH_DOCTOR_BY_ROLE_ID_FAILED,
});
...
export const getPriceStart = () => {
    return async (dispatch, getState) => {
        try {
            ...
            dispatch(fetchPriceFailed());
            console.log('fetchPriceFailed', e);
        }
    }
}
