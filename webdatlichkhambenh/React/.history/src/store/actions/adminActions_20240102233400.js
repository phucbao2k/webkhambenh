...
export const fetchAdminBookingByRoleId = () => {
    return async (dispatch, getState) => {
        try {
            let res = await getAdminBookingByRoleId();

            if (res && res.errCode === 0) {
                dispatch({
                    type: actionTypes.FETCH_ADMIN_BOOKING_BY_ROLE_ID_SUCCESS,
                    dataAdminBooking: res.data
                    //dataDocs được sử dụng ở file adminReducer.js
                })
            ...
            console.log('FETCH_ADMIN_BOOKING_BY_ROLE_ID_FAILED', e)
            dispatch({
                type: actionTypes.FETCH_ADMIN_BOOKING_BY_ROLE_ID_FAILED
            })
        }
