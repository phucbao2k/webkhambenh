...
import {
    ...
    saveScheduleDoctorService, getUserNotDoctors, saveDetailUserNotDoctorService, getDetailInforUserNotDoctor
...
export const fetchAllUsersStart = () => {
    return async (dispatch, getState) => {
        try {
            dispatch({ type: 'FETCH_ALL_USER_START' });

            const res = await getAllUsers("R1");

            if (res && res.errCode === 0) {
                dispatch({ type: 'FETCH_ALL_USER_SUCCESS', payload: res.users.reverse() });
            } else {
                dispatch({ type: 'FETCH_ALL_USER_FAILED', payload: "Failed to fetch all users" });
            }
        } catch (e) {
            dispatch({ type: 'FETCH_ALL_USERS_FAILED', payload: "Failed to fetch all users" });
