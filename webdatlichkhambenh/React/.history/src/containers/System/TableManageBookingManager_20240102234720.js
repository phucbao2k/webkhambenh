...
class TableManageBookingManager extends Component {
    ...
    componentDidMount() {
        this.props.fetchAdminBookingByRoleId('R4');
    }

    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.listUsers !== this.props.listUsers) {
            this.setState({
                listAdminBooking: this.props.listUsers,
            });
        }
    }
...
const mapStateToProps = (state) => {
    return {
        listUsers: state.admin.adminBooking,
    };
};

const mapDispatchToProps = (dispatch) => {
    return {
        fetchAdminBookingByRoleId: (roleId) => dispatch(actions.fetchAdminBookingByRoleId(roleId)),
