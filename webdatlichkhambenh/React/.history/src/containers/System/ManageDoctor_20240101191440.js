...
class ManageDoctor extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selectedPosition: '',
            action: '',
            newPositionId: '', // Thêm dòng này
            newPhoneNumber: '', // Thêm dòng này
        }
    }
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (prevProps.allRequiredDoctorInfor !== this.props.allRequiredDoctorInfor) {
            let { resPayment, resPrice, resProvince, resSpecialty, resClinic, resPosition } = this.props.allRequiredDoctorInfor;
        ...
        if (prevProps.language !== this.props.language) {
            let dataSelect = this.buildDataInputSelect(this.props.allDoctors, 'USERS');
            let { resPrice, resPayment, resProvince, resSpecialty, resClinic, resPosition } = this.props.allRequiredDoctorInfor;
            let dataSelectPrice = this.buildDataInputSelect(resPrice, 'PRICE');
            let dataSelectPayment = this.buildDataInputSelect(resPayment, 'PAYMENT');
            let dataSelectProvince = this.buildDataInputSelect(resProvince, 'PROVINCE');
            let dataSelectSpecialty = this.buildDataInputSelect(resSpecialty, 'SPECIALTY');
        ...
        if (prevProps.allRequiredUserNotDoctorInfor !== this.props.allRequiredUserNotDoctorInfor) {
            let { resPayment, resPrice, resProvince, resSpecialty, resClinic, resPosition } = this.props.allRequiredUserNotDoctorInfor;
            let dataSelectPrice = this.buildDataInputSelect(resPrice, 'PRICE');
            let dataSelectPayment = this.buildDataInputSelect(resPayment, 'PAYMENT');
            let dataSelectProvince = this.buildDataInputSelect(resProvince, 'PROVINCE');
            let dataSelectSpecialty = this.buildDataInputSelect(resSpecialty, 'SPECIALTY');
        ...
        if (prevProps.language !== this.props.language) {
            let dataSelect = this.buildDataInputSelect(this.props.allDoctors, 'USERS');
            let { resPrice, resPayment, resProvince, resSpecialty, resClinic, resPosition } = this.props.allRequiredUserNotDoctorInfor;
            let dataSelectPrice = this.buildDataInputSelect(resPrice, 'PRICE');
            let dataSelectPayment = this.buildDataInputSelect(resPayment, 'PAYMENT');
            let dataSelectProvince = this.buildDataInputSelect(resProvince, 'PROVINCE');
            let dataSelectSpecialty = this.buildDataInputSelect(resSpecialty, 'SPECIALTY');
    ...
    handleSaveContentMarkdown = () => {
        ...
        this.props.saveDetailDoctor({
            ...
            phoneNumber: newPhoneNumber, // Sử dụng giá trị từ state
            newPositionId: newPositionId, // Sử dụng giá trị từ state
            //doctorId: this.state.selectedDoctor.value dùng để gán giá trị item.id cho doctorId rồi lưu vào db
        })
        this.props.updateDoctorDetails({ positionId: newPositionId, phoneNumber: newPhoneNumber });
    }
    ...
    handleChangeSelect = async (selectedDoctor) => {
        ...
        let { listPayment, listPrice, listProvince, listSpecialty, listClinic, listPosition } = this.state;
        let res = await getDetailInforDoctor(selectedDoctor.value);
        if (res && res.errCode === 0 && res.data && res.data.Markdown) {
            let markdown = res.data.Markdown;
            let addressClinic = '', nameClinic = '', positionId = '', paymentId = '',
    ...
    handleChangeSelectUserNotDoctor = async (selectedUserNotDoctor) => {
        this.setState({ selectedUserNotDoctor }, () =>
            console.log(`Doctor selected:`, this.state.selectedUserNotDoctor)
        );
        let { listPayment, listPrice, listProvince, listSpecialty, listClinic, listPosition } = this.state;
