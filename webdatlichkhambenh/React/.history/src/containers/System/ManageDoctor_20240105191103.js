...
class ManageDoctor extends Component {
    ...
    handleSaveContentMarkdown = async () => {
        ...
        console.log('newPositionId:', this.state.newPositionId);
        console.log('newPhoneNumber:', this.state.newPhoneNumber);
   ...
   handleChangeSelect = async (selectedDoctor) => {
    ...
    const { listPrice, listProvince, listSpecialty, listClinic, listPosition } = this.state;
    ...
    if (res && res.errCode === 0 && res.data && res.data.Markdown) {
        ...
        // Tìm giá trị tương ứng trong danh sách
        selectedPrice = listPrice.find(item => item && item.value === priceId) || null;
        selectProvince = listProvince.find(item => item && item.value === provinceId) || null;
        selectedSpecialty = listSpecialty.find(item => item && item.value === specialtyId) || null;
        selectedClinic = listClinic.find(item => item && item.value === clinicId) || null;
        selectedPosition = listPosition.find(item => item && item.value === positionId) || null;

        // Gán giá trị cho newPositionValueVi và newPositionValueEn
        let newPositionValueVi = valueVi || ''; // Thay 'valueVi' bằng trường dữ liệu thực tế
        let newPositionValueEn = valueEn || ''; // Thay 'valueEn' bằng trường dữ liệu thực tế
