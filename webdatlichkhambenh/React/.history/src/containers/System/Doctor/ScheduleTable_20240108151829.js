// HIỂN THỊ CÁC LỊCH HẸN PHẢI KHÁM TRONG NGÀY CỦA BÁC SĨ
...
import { getAllPaidBookingForDoctor, postSendDone,postReBooking } from '../../../services/userService';
...
class ScheduleTable extends Component {
    ...
    getDataPatient = async () => {
        ...
        let res = await getAllPaidBookingForDoctor({
            statusId: status,
            date: formatedDate,
            doctorId: user.id
        })
    ...
    sendDone = async (dataChild) => {
        let { dataModal } = this.state;
        this.setState({
            isShowLoading: true
        })
        let res = await postSendDone({
    ...
    sendReBooking = async (dataChild) => {
        let { dataModal } = this.state;
        this.setState({
            isShowLoading: true
        })
        let res = await postReBooking({
            email: dataChild.email,
            imgBase64: dataChild.imgBase64,
            doctorId: dataModal.doctorId,
            patientId: dataModal.patientId,
    ...
    render() {
        ...
        let { dataPatient, dataModal, isOpenModal, isOpenReBookingModal } = this.state;
            ...
            <>
                <LoadingOverLay active={this.state.isShowLoading}
                    ...
                    <div className="manage-patient-container">
                        <div className="m-p-title">Lịch hẹn phải khám trong ngày</div>
                                                ...
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            ...
                                                            <button className="mp-btn-confirm" onClick={() => this.handleBtnConfirm(item)}>Hoàn thành</button>
                                                            <button className="mp-btn-confirm"
                                                                onClick={() => this.handleBtnReBooking(item)}>Đặt lại lịch hẹn
                                                                </button>
                                                        </td>
                                                    </tr>
                                                )
                    ...
                    <ChangeStatusModal
                        ...
                        sendDone={this.sendDone} />
                    <ReBookingModal
                        isOpenReBookingModal={isOpenReBookingModal}
                        dataModal={dataModal}
                        closeReBookingModal={this.closeReBookingModal}
                        sendReBooking={this.sendReBooking} />
