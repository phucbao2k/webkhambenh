...
class ManageDoctor extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (prevProps.allRequiredUserNotDoctorInfor !== this.props.allRequiredUserNotDoctorInfor) {
            let { resPayment, resPrice, resProvince, resSpecialty, resClinic } = this.props.allRequiredUserNotDoctorInfor;
            let dataSelectPrice = this.buildDataInputSelect(resPrice, 'PRICE');
            let dataSelectPayment = this.buildDataInputSelect(resPayment, 'PAYMENT');
            let dataSelectProvince = this.buildDataInputSelect(resProvince, 'PROVINCE');
            let dataSelectSpecialty = this.buildDataInputSelect(resSpecialty, 'SPECIALTY');
            let dataSelectClinic = this.buildDataInputSelect(resClinic, 'CLINIC');
            console.log('bao phuc check data user new: ', dataSelectPrice, dataSelectPayment, dataSelectProvince,
                dataSelectSpecialty, dataSelectClinic);
            this.setState({
                listPrice: dataSelectPrice,
                listPayment: dataSelectPayment,
        ...
        if (prevProps.language !== this.props.language) {
            let dataSelect = this.buildDataInputSelect(this.props.allDoctors, 'USERS');
            let { resPrice, resPayment, resProvince, resSpecialty, resClinic } = this.props.allRequiredUserNotDoctorInfor;
            let dataSelectPrice = this.buildDataInputSelect(resPrice, 'PRICE');
            let dataSelectPayment = this.buildDataInputSelect(resPayment, 'PAYMENT');
            let dataSelectProvince = this.buildDataInputSelect(resProvince, 'PROVINCE');
            let dataSelectSpecialty = this.buildDataInputSelect(resSpecialty, 'SPECIALTY');
    ...
    handleSaveContentMarkdown = () => {
        ...
        let { hasOldData, selectedUserNotDoctor } = this.state;

        // Kiểm tra xem có thông tin người không phải bác sĩ nào được chọn hay không
        if (selectedUserNotDoctor) {
            // Gọi API để lấy thông tin chi tiết của người không phải bác sĩ
            getDetailInforUserNotDoctor(selectedUserNotDoctor.value)
                .then(res => {
                    if (res && res.errCode === 0) {
                        // Nếu có, thêm thông tin mới vào danh sách bác sĩ
                        this.props.saveDetailDoctor({
                            contentHTML: this.state.contentHTML,
                            contentMarkdown: this.state.contentMarkdown,
                            description: this.state.description,
                            doctorId: selectedUserNotDoctor.value,
                        ...
                        // Loại bỏ người không phải bác sĩ đã được thêm vào danh sách bác sĩ
                        let updatedListUsersNotDoctor = this.state.listUsersNotDoctor.filter(
                            user => user.value !== selectedUserNotDoctor.value
                        );

                        this.setState({
                            listUsersNotDoctor: updatedListUsersNotDoctor,
                        });
                    }
                })
                .catch(error => {
    ...
    handleChangeSelectUserNotDoctor = (selectedUser) => {
        console.log("Selected User:", selectedUser);
        console.log("List Users Not Doctor:", this.state.listUsersNotDoctor);
        this.setState({
            selectedUserNotDoctor: selectedUser,
        });
    };
