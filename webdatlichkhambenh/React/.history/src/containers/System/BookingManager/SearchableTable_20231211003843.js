...
class SearchableTable extends Component {
    ...
    handleSearch = async () => {
        try {
            ...
            if (searchTerm.trim() !== "") {
                const response = await getSearchBookingForAdminBooking({
                ...
                if (response.errCode === 0) {
                    ...
                    const isDataChanged = !this.areSearchResultsEqual(data, this.state.searchResults);
    ...
    areSearchResultsEqual = (data1, data2) => {
        // Xác định cách so sánh dữ liệu, ví dụ: dựa trên thuộc tính id
        const getKey = item => item.id;

        // Kiểm tra nếu cả hai đối số đều không phải là mảng
        if (!Array.isArray(data1) || !Array.isArray(data2)) {
            return false;
        }

        // Lấy danh sách key từ cả hai mảng
        const keys1 = data1.map(getKey);
        const keys2 = data2.map(getKey);

        // So sánh danh sách key
        const keySetsAreEqual = new Set(keys1).size === new Set([...keys1, ...keys2]).size;

        // Nếu danh sách key không giống nhau, thì dữ liệu khác nhau
        return keySetsAreEqual;
    };
                            ...
                            <div className="col-12 table-manage-patient">
                                <table style={{ width: '100%' }}>
                                    <tbody>
                                        <tr>
                                            <th>
                                                <FormattedMessage id="patient.booking-modal.numerical-order" />
                                            </th>
                                            <th>
                                                <FormattedMessage id="patient.booking-modal.doctorName" />
                                            </th>
                                            <th>
                                            ...
                                            <th>
                                                <FormattedMessage id="patient.booking-modal.avatar" />
                                            </th>
                                            <th>Actions</th>
                                        </tr>
