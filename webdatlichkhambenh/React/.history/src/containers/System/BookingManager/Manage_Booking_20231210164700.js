...
class Manage_Booking extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            searchTerm: '',
            searchResults: [],
            // Thêm mảng tạm thời để lưu trữ tất cả dữ liệu
            allData: [],
        }
    }
    ...
    getDataPatient = async () => {
        ...
        if (res && res.errCode === 0) {
            this.setState({
                ...
                // Cập nhật mảng tạm thời với toàn bộ dữ liệu
                allData: res.data,
            });
        }
    }
    ...
    handleSearch = async () => {
        try {
            ...
            // Thực hiện tìm kiếm trên mảng tạm thời
                                        ...
                                        {(searchResults && searchResults.length > 0) ?
                                            ...
                                            (dataPatient && dataPatient.length > 0) ?
                                                dataPatient.map((item, index) => {
                                                    // Hiển thị toàn bộ dữ liệu khi không có tìm kiếm
                                                    let time = language === LANGUAGES.VI ?
                                                        item.timeTypeDataPatient.valueVi : item.timeTypeDataPatient.valueEn;
                                                    return (
                                                        <tr key={index}>
