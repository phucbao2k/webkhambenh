...
import SearchableTable from '../Admin/SearchableTable';  // Import SearchableTable

class Manage_Booking extends SearchableTable {  // Mở rộng từ SearchableTable
    constructor(props) {
        super(props);
        this.state = {
    ...
    handleSearch = async () => {
        try {
            const { searchTerm, allData } = this.state;

            if (!searchTerm.trim()) {
                this.setState({ searchResults: allData });
                return;
            }
