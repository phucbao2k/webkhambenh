// GỌI CÁC THÔNG TIN TỪ LIST USER PHÍA DƯỚI, TIẾN HÀNH CHỈNH SỬA TÙY Ý, CÓ GỌI SẴN VÀI THÔNG TIN KHÁC
...
import { updateUserDetails } from '../../store/actions/adminActions.js';
  ...
  handleSaveUser = async () => {
    ...
    if (action === CRUD_ACTIONS.CREATE) {
      this.props.createNewUser({
        ...
        phoneNumber: newPhoneNumber, // Sử dụng newPhoneNumber ở đây
    ...
    if (action === CRUD_ACTIONS.EDIT) {
      errors.length = this.handleSubmit(); // Bạn không cần kiểm tra lỗi lần thứ hai ở đây
      if (errors.length > 0) return;
      ...
      this.props.editUserRedux({
        ...
        gender: this.state.gender,
        roleId: this.state.role,
        positionId: this.state.position, // Có thể bạn cần sửa thành positionId nếu đây là tên của trường trong dữ liệu đầu vào
        avatar: this.state.avatar,
      })
    }

    this.props.updateUserDetails({ positionId: newPositionId, phoneNumber: newPhoneNumber });
  }
