...
class TableManageAdmin extends Component {
    ...
    componentDidMount() {
        this.props.fetchAdminByRoleId();
    }

    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.admin !== this.props.admin) {
            this.setState({
                listAdmin: this.props.admin,
            });
        }
    }
    ...
    render() {
        let arrUsers = this.state.listAdmin;

        return (
            <React.Fragment>
                <div className="users-container">
                    <div className="title text-center">Manage Admin</div>
...
const mapDispatchToProps = (dispatch) => {
    return {
        fetchAdminByRoleId: () => dispatch(actions.fetchAdminByRoleId(roleId)),
