...
class VNPayPaymentButton extends Component {
    ...
    handlePayment = async () => {
        try {
            ...
            if (!this.isValidNumber(amountEn)) {
                console.error('Invalid amount value:', amountEn);
                return;
            }

            const amount = language === LANGUAGES.VI ? Number(amountVi) : Number(amountEn);
            ...
            }, 300000); // 300,000 milliseconds = 300 seconds (5 minutes)
        } catch (error) {
