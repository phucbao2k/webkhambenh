            ...
            <div>
                {selectedDoctor && (
                    <>
                        ...
                        {selectedDoctor.doctors.map((doctors, index) => (
                            <div key={index}>
                                <p>Địa chỉ cơ sở Y tế: {doctors.addressClinic}</p>
                                <p>Tên cơ sở Y tế: {doctors.nameClinic}</p>

                                {/* Kiểm tra xem priceTypeData có tồn tại không */}
                                {doctors.priceTypeData && (
                                    <p>Loại giá (VND): {doctors.priceTypeData.valueVi}</p>
                                )}

                                {/* Kiểm tra xem provinceTypeData có tồn tại không */}
                                {doctors.provinceTypeData && (
                                    <p>Tỉnh/Thành phố: {doctors.provinceTypeData.valueVi}</p>
                                )}

                                {/* Kiểm tra xem User có tồn tại không trước khi truy cập */}
                                {doctors.User && (
                                    <>
                                        <p>Họ và Tên: {doctors.User.firstName} {doctors.User.lastName}</p>
                                    </>
                                )}

                                {doctors.specialityData && (
                                    <p>Tên chuyên khoa: {doctors.specialityData.name}</p>
                                )}
                            </div>
                        ))}
