...
app.post('/api/get-doctors-by-position', async (req, res) => {
    ...
    try {
        ...
        const result = await Doctor_Infor.findAll({
            ...
            attributes: {
                exclude: ['id', 'doctorId', 'specialtyId'], // Thêm 'specialtyId' vào danh sách exclude
            },
            raw: true,
        });
        ...
        const mappedResult = result.map(doctor => ({
            ...
            specialityData: {
                name: doctor['specialityData.name'] || null, // Đổi từ doctor.specialityData?.name sang doctor['specialityData.name']
            },
        }));
