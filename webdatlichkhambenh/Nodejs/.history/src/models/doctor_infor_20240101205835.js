...
export default (sequelize) => {
    ...
    Doctor_Infor.beforeUpdate(async (instance) => {
        try {
            const [rowsUpdated, [updatedUser]] = await sequelize.models.User.update(
                { positionId: instance.positionId },
                { returning: true, where: { id: instance.doctorId } }
            );

            if (rowsUpdated > 0) {
                console.log(`Đã cập nhật thông tin cho User: ${updatedUser.id}`);
            } else {
