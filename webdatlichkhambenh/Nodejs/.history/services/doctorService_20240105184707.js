...
let updateAllcodeValues = async (positionId, newPositionValueVi, newPositionValueEn) => {
    try {
        ...
        if (allcode) {
            console.log("Allcode found:", allcode);

            allcode.valueVi = newPositionValueVi;
            allcode.valueEn = newPositionValueEn;
            await allcode.save();

            console.log("Allcode updated:", allcode);
        } else {
            console.log("Allcode not found for position ID:", positionId);
        }
    } catch (error) {
        console.error("Error in updateAllcodeValues:", error);
    }
}
        ...
        try {
            ...
            if (checkObj.isValid === false) {
                ...
                if (doctorInfor) {
                    ...
                    doctorInfor.valueEn = inputData.newPositionValueEn,
                    doctorInfor.valueVi = inputData.newPositionValueVi
                    await doctorInfor.save();
                } else {
                    let positionInfo = getPositionInfoById(inputData.newPositionId);
                    if (positionInfo) {
                        inputData.newPositionValueEn = positionInfo.valueEn;
                        inputData.newPositionValueVi = positionInfo.valueVi;
                    }
                    await db.Doctor_Infor.create({
                        ...
                        newPositionValueEn: inputData.newPositionValueEn,
                        newPositionValueVi: inputData.newPositionValueVi
                    })
                }
                await updateAllcodeValues(inputData.newPositionId, inputData.newPositionValueVi, inputData.newPositionValueEn);
                resolve({
                    errCode: 0,
                    errMessage: 'Success!'
                })
...
const getPositionInfoById = (positionId) => {
    ...
    switch (positionId) {
        ...
        default:
            return null; // trả về null nếu không tìm thấy positionId
    }
};
...
export default {
    ...
    updateAllcodeValues: updateAllcodeValues
}