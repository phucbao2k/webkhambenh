...
let getUsersByRoleId = (roleId) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            resolve(users || []); // Trả về mảng rỗng nếu không có người dùng
        } catch (e) {
            reject(e);
        }
    });
...
let getAdminByRoleId = (roleId) => {
    return new Promise(async (resolve, reject) => {
        try {
            let admin = '';

            if (roleId === 'R1') {
                admin = await db.User.findAll({
                    attributes: {
                        exclude: ['password']
                    }
                });
...
let getAdminBookingByRoleId = (roleId) => {
    return new Promise(async (resolve, reject) => {
        try {
            let adminBooking = '';

            if (roleId === 'R4') {
                adminBooking = await db.User.findAll({
                    attributes: {
                        exclude: ['password']
                    }
                });
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (user) {
                ...
                user.positionId = data.newPositionId;
                ...
                if (doctorInfor) {
                    doctorInfor.positionId = data.newPositionId;
                    doctorInfor.phoneNumber = data.newPhoneNumber;
...
export default {
    ...
    getAdminByRoleId: getAdminByRoleId,
    getAdminBookingByRoleId: getAdminBookingByRoleId,
    getDoctorByRoleId: getDoctorByRoleId,
}