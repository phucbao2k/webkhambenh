...
const updateField = async (model, recordId, fields, values) => {
    try {
        const updateObject = {};
...
let checkRequiredFields = (inputData) => {
    let arrFields = ['doctorId', 'contentHTML', 'contentMarkdown', 'action',
        ...
        'nameClinic', 'addressClinic', 'selectedPosition', 'specialtyId', 'clinicId', 'phoneNumber'];
    let isValid = true;
    let element = '';
...
const updateField = async (model, userId, fields, values) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!userId || !fields || !values || fields.length !== values.length) {
                resolve({
                    errCode: 1,
            ...
            // Tìm thông tin người dùng hoặc bác sĩ
            ...
            if (!user) {
                resolve({
                    errCode: 1,
                    errMessage: `${model.getTableName()} not found`
                });
                return;
            }
            ...
            resolve({
                errCode: 0,
                errMessage: `Update ${model.getTableName()} information successfully`
            });
        } catch (error) {
            console.log(error);
            resolve({
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (checkObj.isValid === false) {
                ...
                await updateField(db.User, inputData.doctorId, ['phoneNumber', 'positionId'], [inputData.phoneNumber, inputData.selectedPosition]);
