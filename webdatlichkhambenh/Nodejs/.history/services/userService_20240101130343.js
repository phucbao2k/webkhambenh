...
const updateField = async (model, userId, fields, values) => {
    try {
        const updateObject = {};
        ...
        const result = await model.update(updateObject, {
            where: { id: userId },
            returning: true,
            plain: true,
        });
        ...
        return {
            success: false,
            error: error.message || "Error updating fields",
        };
    }
};
...
const executeUpdate = async () => {
    const userResult = await updateField(db.User, userId, ['phoneNumber', 'positionId'], [newPhoneNumber, newPositionId]);

    // Sử dụng hàm updateField cho Doctor_Infor
    const doctorResult = await updateField(db.Doctor_Infor, doctorId, ['phoneNumber', 'positionId'], [newPhoneNumber, newPositionId]);
...
// Gọi hàm executeUpdate
executeUpdate();
const userModel = db.User;
const userResult = await updateField(userModel, userId, ['phoneNumber', 'positionId'], [newPhoneNumber, newPositionId]);

// Sử dụng hàm updateField cho Doctor_Infor
const doctorModel = db.Doctor_Infor;
const doctorResult = await updateField(doctorModel, doctorId, ['phoneNumber', 'positionId'], [newPhoneNumber, newPositionId]);
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (checkObj.isValid === false) {
                ...
                if (doctorInfor) {
                    // Bổ sung điều kiện để xác định xem có sẵn `doctorId` hay không
                    if (inputData.doctorId) {
                        ...
                        // Thêm xử lý cho trường hợp không có `doctorId`
                        // Ví dụ: Tạo một thông báo lỗi hoặc xử lý khác
                        resolve({
                            errCode: 1,
                            errMessage: 'Invalid doctor or user input'
                        });
                        return;
                    }
                } else {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (user) {
                ...
                // Update thông tin Doctor_Infor
                await updateField(db.Doctor_Infor, data.id, ['phoneNumber', 'positionId'], [data.phoneNumber, data.positionId]);
