...
app.post('/create_payment_url', (req, res, next) => {
    try {
        ...
        const returnUrl = 'http://localhost:7070/order/vnpay_return';
        ...
        const orderInfo = req.body.orderDescription || 'Payment for something';
        ...
        // Trả về URL để chuyển hướng ở phía client
        const finalVnpUrl = vnpUrl + '?' + qs.stringify(vnp_Params, { encode: false });
        res.json({ redirectUrl: finalVnpUrl });
    } catch (error) {
...
app.get('/vnpay_return', function (req, res, next) {
    try {
        ...
        const hmac = createHmac("sha", secretKey);
